...
<ul class="breadcrumb">
	<li><a href="#">Home</a></li>
	<li class="active">Bihar PDS Route Optimisation</li>
</ul>
<!-- END BREADCRUMB -->
<div>
					...
					<div class="panel-body">
						...
						<div class="row">
							...
							<div class="row">
								...
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										style="background-color:#FF8C00; color:white; padding:12px; font-weight: bold;">
										<div style="font-size:20px" id="total_demand"></div>
										<div style="font-size:14px">Total Warehouse Wheat Demands (Qtl)</div>
									</div>
								</div>
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										...
										<div style="font-size:14px">Total Warehouse Rice Demands (Qtl)</div>
									</div>
								</div>
								<div class="col-md-4 mb-4">
									<div class="card h-100"
								...
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										style="background-color:#CC00CC; color:white; padding:12px; font-weight: bold;">
										<div style="font-size:20px" id="total_dcp"></div>
										<div style="font-size:14px">Total DCP </div>
									</div>
								</div>
								
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										style="background-color:#CCCC0C; color:white; padding:12px; font-weight: bold;">
										<div style="font-size:20px" id="total_riceprocurment"></div>
										<div style="font-size:14px">Total Rice procurement (Qtl)</div>
									</div>
								</div>
								
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										style="background-color:#66FFFF; color:white; padding:12px; font-weight: bold;">
										<div style="font-size:20px" id="total_wheatprocurment"></div>
										<div style="font-size:14px">Total Wheat procurement (Qtl)</div>
									</div>
								</div>
...
<script>
						...
						.then(data => {
							...
							fetch(pythonUrl + 'getfcidataleg1', {
								...
								.then(data => {
									...
									document.getElementById("total_dcp").innerHTML = formatNumberWithCommasWithoutDecimal(data["DCP_No"]);
									document.getElementById("total_riceprocurment").innerHTML = formatNumberWithCommasWithoutDecimal(data["Rice_Procurement"]);
									document.getElementById("total_wheatprocurment").innerHTML = formatNumberWithCommasWithoutDecimal(data["Wheat_Procurement"]);
	...
	function generateoptimizedplan() {
		...
		formData.append('client_id', document.getElementById("username").value);
		document.getElementById("processingPopup").style.display = "flex";
		...
		.then(data => {
			if(data.status == 1) {
				...
				if(job.status == 'completed') {
					...
					alert("Process " + job.status + (job.message ? ": " + job.message : ""));
					resetUI();
				} else {
	...
	function checkActiveJob() {
		...
		if(!clientId) return;
		fetch(pythonUrl + 'active_job?client_id=' + clientId + '&endpoint=/processFileleg1')
		.then(r => r.json())
		.then(data => {
	...
	if (checkbox.checked) {
		...
		fetch(pythonUrl + 'getGraphDataleg1', {
			...
			.then(data => {
				...
				var totalDemand = totalDemand + totalDemandRice;
				...
				if (totalCapacity > 0 && totalDemand > 0) {
					...
					var procurement_rice = unionArray.map(district => data.State_Riceprocurement[district]);
					var procurement_wheat = unionArray.map(district => data.State_Wheatprocurement[district]);
					...
					var newData = {
						...
						datasets: [
							...
							},
							{
								...
								data: procurement_rice
							},
							{
								label: 'Wheat Procurement',
								backgroundColor: '#FF8000',
								data: procurement_wheat
							},
							{
								label: 'Wheat Supply',
