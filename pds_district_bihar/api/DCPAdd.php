...
if(password_verify($person->getPassword(), $dbHashedPassword)){
    ...
    if(!isValidCoordinate($_POST["latitude"], 'latitude')){
        echo "Error : Latitude must be a valid number between -90 and 90";
        exit();
    }
    if(!isValidCoordinate($_POST["longitude"], 'longitude')){
        echo "Error : Longitude must be a valid number between -180 and 180";
        exit();
    }
    ...
    if($numrows_insert==0){
		...
		writeLog("district_user ->" ." DCP added ->". $_SESSION['district_user'] . "| Requested JSON -> " . json_encode($filteredPost));
